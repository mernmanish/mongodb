...
@Component({
  ...
  styleUrls: ['./apply-form.component.css', './apply-form.component.scss']
})
export class ApplyFormComponent implements OnInit {
  isCappingValueDets:any=0;
  totalPreviousAmount:any =0;
  claimPeriodValue: any = 0;
  loginSts: any;
  ...
  hcPrevClaimedAmount: any = 0;
  ptsToatlClaimedAmount: any = 0;
  hcToatlClaimedAmount: any = 0;
  hcIncentiveClaimed:any =0;
  intNoOfDateField: any = 0;
  booleanPrevClaimStatus: any = true;
  totalRepaymentInterAmount: any = 0;
  hcEligibleAmount:any = 0;
  formEditStatus: any = 0;
  financialYearsWithQuarters: FinancialYear[] = [];
  amountAddMoreDet:any =0;
  amountAddMoreDetSecond:any =0;
  skillUpgradeAddmoreCount:any =0;
  ...
  userId: any;
  skillUpgradePrevAmount: number;
  suiAddMoreCount: number = 0;
  suiAddMoreCtrlIds: any = [];
  ...
  // }
  // Modified by : Added by Arpita || On: 24-jul-24
  loadDependCtrls() {
  ...
  loadDynDepend(ctrlId: any, typeId: any = 0) {
    ...
    for (let dynbndtype of bnddpndfld) {
      ...
      if (dynbndtype.getAttribute('data-dynbinddependflag') == 'true') {
        ...
        if (this.arrallCascadingDetails[dynCtrlId].ctrlCCDTableName){
        this.dynmaicValApi(parms, dynCtrlId);
      }
      }
    }
  ...
  doSchemeApply() {
     ...
     const allDataValid = this.finalApplyCustomValidations();
     if (!allDataValid) {
         return;
     }
  ...
  saveFileTemp(event: any, fileId: any, fileType: any, fileSize: any, fileForApproval: any, fileSizeType: any) {
    const target = event.target as HTMLInputElement;
    const files: any = target.files as FileList;
    if (files) {
      const uploadedFileSize = files[0].size;
      ...
      if (!this.vldChkLst.validateFileSize(uploadedFileSize, fileSize, fileSizeType)) {
        let fileSizeMsg = '';
      ...
      const uploadChunk = () => {
        ...
        formData.append('fileName', sessionStorage.getItem('REGD_ID')+'_'+file.name );
        formData.append('fileType', fileType);
        formData.append('fileSize', fileSize);
        formData.append('fileSizeType', fileSizeType);
  
        this.WebCommonService.chunkFileUpload(formData).subscribe({
          next: (res: any) => {
            if (res.status === 200) {
              currentChunk++;
  ...
  */
  ...
  addMoreData(addMorectrlId: any) {
    ...
    let suiAddMoreClassList = document.getElementById(suiAddMoreId);
    if(suiAddMoreClassList.classList.contains("cls_sui_amount") && this.suiAddMoreCount >= 5){
      return;
    }else if(suiAddMoreClassList.classList.contains("cls_sui_amount")){
        this.suiAddMoreCount++;
        this.suiAddMoreCtrlIds.push(addMorectrlId);
    }
    let validateArray: any[] = [];
    let validatonStatus = true;
    ...
    let otherAssIdLenThree = '';
    if(this.arrAddmoreFilledData[addMorectrlId][this.arrAddmoreFilledData[addMorectrlId].length - 1].length >= 3){
    otherAssIdLenThree = this.arrAddmoreFilledData[addMorectrlId][this.arrAddmoreFilledData[addMorectrlId].length - 1][3].ctrlId;
    }
    ...
    let prevClassListThree = document.getElementById(otherAssIdLenThree);

    if(otherAssClassList.classList.contains("cls_iSOtherAssAmt") || otherAssClassList.classList.contains("cls_ivsOtherAsset") || otherAssClassList.classList.contains("cls_hcisEmp") || prevClassListThree.classList.contains("cls_perEmpAmount") || prevClassList.classList.contains("cls_sui_amount") || otherAssClassList.classList.contains("cls_course_fee_per_emp")){
  ...
  setDynRadioBtn(dynSetVal: any, ctrlValue: any) {
    ...
    if (dynSetVal != null) {
     ...
     // return dynSetVal.toString() === ctrlValue.toString();
    }
    else {
      return false;
    }
  ...
  deleteAddMore(event: any, ctrlId: any, indx: any) {
    if(this.isAddMoreCtrlIds.includes(ctrlId)){
      ...
      this.interestSubsidyAddMoreCalculation(this.arrAddmoreFilledData, ctrlId, this.formName);
    }else{
      if (this.suiAddMoreCtrlIds.includes(ctrlId)) {
        this.suiAddMoreCtrlIds.splice(indx, 1);
        this.suiAddMoreCount = this.suiAddMoreCount - 1;
      }
      this.arrAddmoreFilledData[ctrlId].splice(indx, 1);
  ...
  fillAddMoreArray(addMorectrlId: any, addMoreFormConfigData: any, addMoreFormResult: any, addMorectrlClass: any) // when page is loaded this function set's add more array
  {
     if (addMoreFormResult[addMorectrlId] != undefined && !(Object.keys(this.arrAddmoreElemntKeys)).includes(addMorectrlId)) {
      ...
      //code added by Bindurekha Nayak 12-06-2024
      if (addMoreFormResult[addMorectrlId]['addMoreDataValue'] != '') {
        if(this.processId == 68 && addMorectrlClass == 'cls_other_assets'){
          let amountAddMore = 0; 
          for (let addmoreloop of addMoreFormResult[addMorectrlId]['addMoreDataValue']) {
            if(addmoreloop.int_amount>0){
              const parsedAmount = parseFloat(addmoreloop.int_amount);
        ...
        if(this.processId == 68 && addMorectrlClass == 'cls_det_termLoan'){
          this.isAddMoreCtrlIds.push(addMorectrlId);
        }
        if(this.intProductId == 111 && addMorectrlClass == 'cls_certifying_body'){
          for (let addmoreloop of addMoreFormResult[addMorectrlId]['addMoreDataValue']) {
          this.skillUpgradeAddmoreCount++;}
          console.log('skillUpgradeAddmoreCount',this.skillUpgradeAddmoreCount);
        }
       //End code  by Bindurekha Nayak 12-06-2024
  ...
  */
  ...
  const claimedAmountField: any = (<HTMLInputElement>document.getElementsByClassName('cls_fca_d_none')[0]);
    ...
    if (genericCompNameElement) {
        genericCompNameElement.appendChild(document.createTextNode(' '+'"' + autoFillUpData['0'].vch_company_name + '"'));
    }
    ...
    for (let addMoreTrTabularLoop of arrAllAddMoreData['addmorerowdata']) //TR Loop
    {
      ...
      for (let addMoreTdTabularLoop of arrAllAddMoreData['addmoreDetails']) // TD Loop
      {
        ...
        else if (addMoreTdTabularLoop['ctrlTypeId'] == 2) // Textbox Validation
        {
          ...
          else if (addMoreTdTabularLoop['ctrlAttributeType'] == 'telephoneNo') // For Valid Mobile
          {
  ...
  */
      ...
      toDate.addEventListener('change', () => {
        if (fromDate.value) {
          if (fromDate.value > toDate.value) {
            ...
            Swal.fire({
              icon: 'error',
              html: "You <b>" + toDateLabel + "</b> cannot be earlier than the <b>" + fromDateLabel + "</b>."
            });
          }
        }
      });
        ...
        if (toDate.value) {
          if (fromDate.value > toDate.value) {
            ...
            Swal.fire({
              ...
              html: "The <b>" + fromDateLabel + "</b> cannot be later than the <b>" + toDateLabel + "</b>."
            });
          }
        }
      });
  ...
  autoCalculation() {
    ...
    const incorporationdate = companyData.length > 0?companyData[0].dtm_incorporation_date:null;
    const gender = companyData.length > 0?companyData[0].int_gender:'';
    const socialcategory = companyData.length > 0?companyData[0].int_social_category:'';
    const disabled = companyData.length > 0?companyData[0].int_physically_handicapped:'';
    // }
    ...
    let total: any = 0;
    let intId: any = sessionStorage.getItem('REGD_ID');
    let vfxParam = {
    ...
    console.log(h4Text +'--' +'[ProductId=='+this.intProductId+']');
    this.processId = parseInt(this.processId);
    switch (this.processId) {
      /*Start of calculation for Lease Rental Reimbursement  New Functionality developed by bindurekha Nayak 27-06-2024*/
      case 57:
        
        this.getOperationLocationDetails(sessionStorage.getItem('REGD_ID'));
          let totalRentaSubsidy: any = (<HTMLInputElement>document.getElementsByClassName('cls_total_subsidy_amount')[0]);
          let claimPeriodElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_claimPeriod')[0]);
          let rentalstartdate: any = (<HTMLInputElement>document.getElementsByClassName('cls_rental_start')[0]);
            let rentalenddate: any = (<HTMLInputElement>document.getElementsByClassName('cls_rental_end')[0]);
          // totalRentaSubsidy.disabled = true;
          let leasedSpace: any = (<HTMLInputElement>document.getElementsByClassName('cls_leased_area')[0]);
          
          
          leasedSpace.addEventListener('input', function () {
            if (leasedSpace.value / empcount > 50) {
              totalSubsidableSpace = empcount * 50;
            }
            else {
              totalSubsidableSpace = leasedSpace.value;
            }
          });
          ...
          rentalstartdate.addEventListener('change', () => {
            ...
            let startDate = new Date(rentalstartdate.value);
            let endDate = new Date(rentalenddate.value);
            if (endDate < startDate) {
              rentalenddate.value = '';
                ...
                let prvParam = {
                  ...
                  "profileId": sessionInfo.USER_ID,
                  "incStartDate": rentalstartdate.value,
                  "claimLimit": 5,
                };
                this.loading = true;
                this.IrmsDetailsService.getyearCheckIncubationRental(prvParam).subscribe(res => {
                  ...
                  if (res.status == 2) {
                    ...
                    if (claimPeriod > ClaimablePeriod) {
                      ...
                      let fromDate = new Date(rentalstartdate.value);
                      ...
                      if (toDate <= currentDate) {
                        ...
                        rentalenddate.value = formattedToDate;
                        rentalenddate.readOnly = true;
                        rentalenddate.dispatchEvent(new Event('change'));
                      } else {
                        Swal.fire({
                          icon: 'error',
          ...
          let paidrental: any = (<HTMLInputElement>document.getElementsByClassName('cls_paid_rental')[0]);
          
          paidrental.addEventListener('input', function () {
            let claimPeriod = parseInt(claimPeriodElement.value);
            let claimMaxValue:any =0;
            let claimMaxValueDet :any =0;
            if(claimPeriod>0){
              this.claimMaxValue=claimPeriod*1200000;
              this.claimMaxValueDet=claimPeriod*500000;
            }
            let rentsubsidy1: any = (paidrental.value * 0.75 <= this.claimMaxValue) ? paidrental.value * 0.75 : this.claimMaxValue;
            let rentsubsidypermonth: any = totalSubsidableSpace * 20;
            let rentalstartdate: any = (<HTMLInputElement>document.getElementsByClassName('cls_rental_start')[0]).value;
            let rentalenddate: any = (<HTMLInputElement>document.getElementsByClassName('cls_rental_end')[0]).value;
            if (rentalstartdate == '' || rentalstartdate == undefined) {
              Swal.fire({
                icon: 'error',
                text: 'Please enter rental start date first'
              });
              paidrental.value = '';
            }
            else if (rentalenddate == '' || rentalenddate == undefined) {
              Swal.fire({
                icon: 'error',
                text: 'Please enter rental end date first'
              });
              paidrental.value = '';
            }
            else {
              const yearsDifference = new Date(rentalenddate).getFullYear() - new Date(rentalstartdate).getFullYear();
              const rentalStartDate = new Date(rentalstartdate);
              const rentalEndDate = new Date(rentalenddate);

              const diffmonths = (rentalEndDate.getFullYear() - rentalStartDate.getFullYear()) * 12 + (rentalEndDate.getMonth() - rentalStartDate.getMonth());
              let rentsubsidy2: any = rentsubsidypermonth * diffmonths;
              subsidy = (rentsubsidy1 <= rentsubsidy2) ? rentsubsidy1 : rentsubsidy2;
              
            }
            if (socialcategory == 2 || socialcategory == 3 || gender == 2 || disabled == 1) {

              let subsidy2: any = (paidrental.value * 0.25 < this.claimMaxValueDet) ? paidrental.value * 0.25 : this.claimMaxValueDet;
              finalsubsidy = subsidy + subsidy2;
              
            }
            else {
              finalsubsidy = subsidy;
            }
            (<HTMLInputElement>document.getElementsByClassName('cls_total_subsidy_amount')[0]).value = String(finalsubsidy);
          });

        
        let towerElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_tower_list_hd')[0]);
      ...
      /*Start of calculation for Interest Subsidy IT Policy New Development add by Bindurekha Nayak Date: 28-06-2024*/
      case 68:{
        this.fillUpMdInfo('cls_iSMdName', 'cls_iSMdDesg');
        ...
        let claimPeriodElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_claimPeriod')[0]);
        this.otherAssetTotalAmount = (this.amountAddMoreDet =='') ? 0 : this.amountAddMoreDet;
        ...
        const headingBtnDiv = financialTxtElement.closest('.cls_det_termLoan')?.querySelector('.heading-btn');
       ...
       this.IrmsDetailsService.getPastRecordEmpTotal(param).subscribe(res => {
         ...
         if (this.totalPreviousAmount == 500000000) {
          Swal.fire({
               icon: 'error',
               text: 'Your  claim amount limit is exceeded, you cannot apply!'
           });
           repaymentDateelement.value='';
         }
       });
        this.fillUpPrevDetails('cls_isPreviousClaimHistory');
        ...
        repaymentToDateelement.addEventListener('change',  () => {
          ...
          let isCappingValueDet = ((500000000 - totalPreviousAmount) <= 100000000) ? 500000000 - totalPreviousAmount : 100000000;
          ...
          this.IrmsDetailsService.getFounderPercentageValue(enterprenureParam).subscribe(res => {
              let repaymentValue = parseFloat(repaymentElement.value); // Ensure the repayment value is a number
              if (res.eligible === 1) {
                this.isCappingValue = claimPeriod * (isCappingValueDet + (isCappingValueDet * 0.01));
                totalRepaymentelement.value = claimPeriod * ((repaymentValue * 0.05) + (repaymentValue * 0.01));
              } else {
                totalRepaymentelement.value = claimPeriod * (repaymentValue * 0.05);
                this.isCappingValue = claimPeriod * isCappingValueDet;
                
              }
      
              // Ensure totalRepaymentelement.value is updated within the subscription block
              totalRepaymentelement.value = (parseFloat(totalRepaymentelement.value) > this.isCappingValue) ? this.isCappingValue : totalRepaymentelement.value;
          });
      });
      this.fromToDateValidation();
      this.alphaNumericValidation();
      ...
      /*Start of calculation for Investment Subsidy */
      case 32:
        ...
        ivsSubCapitalElement.addEventListener('input', () => {
          ...
          let totalAmountVal = (30/100) * ivsCapitalTotal;
          let cappingValue = 30000000;
          ivsClaimedElement.value = (totalAmountVal <= cappingValue) ? totalAmountVal : cappingValue;
        });
        ...
        this.fillUpMdInfo('cls_ivsName', 'cls_ivsDesignation');
        this.alphaNumericValidation();
        break;
      /*End of calculation for Investment Subsidy */


      /*Start of calculation for Quality / Security Certification Fee Reimbursement */
      case 51:
        let qscfParam = {
          "onlineProcessId": this.processId,
        ...
        this.alphaNumericValidation();
      break;
      /*End of calculation for Quality / Security Certification Fee Reimbursement */

      /*Start of calculation for State Component of Net GST  year functionality added by Bindurekha nayak08-07-2024*/
      case 36:{
        this.checkFieldAmount('cls_gst_paid ','cls_fca');
        let sgstParam = {
          "onlineProcessId": this.processId,
          "profileId": this.userId,
          "intId": sessionStorage.getItem('REGD_ID')
        ...
        //implementation of uder bda area billig period By:Sibananda sahu

        let underBdaArea : any = (<HTMLInputElement>document.getElementsByClassName('cls_bda_area')[0]);
        let billingStart : any = (<HTMLInputElement>document.getElementsByClassName('cls_billing_start')[0]);
        let billingEnd : any = (<HTMLInputElement>document.getElementsByClassName('cls_billing_end')[0]);

        underBdaArea.addEventListener('input', function () {
          billingStart.value = '';
          billingEnd.value = '';
          if(underBdaArea.value == 0){
            Swal.fire({
                  icon: 'error',
                  text: 'Please select wheather under BDA Area or not'
                })
            billingStart.value = '';
            billingEnd.value = '';    
          }
        ...
        billingEnd.addEventListener('input', () => {
         let underBdaAreaValue = underBdaArea.value;
            if(underBdaAreaValue == 0){
              Swal.fire({
                    icon: 'error',
                    text: 'Please select wheather under BDA Area or not'
                  })
              billingStart.value = '';
              billingEnd.value = '';
            } else if(underBdaAreaValue == 1 || underBdaAreaValue == 2) {
              let billingStartValue = billingStart.value;
              let billingEndValue = billingEnd.value;
              if(billingStartValue && billingEndValue){
                let diffInMonths = (new Date(billingEndValue).getMonth() - new Date(billingStartValue).getMonth()) + (new Date(billingEndValue).getFullYear() - new Date(billingStartValue).getFullYear()) * 12;
                let diffYear = diffInMonths / 12;
               if (underBdaAreaValue == 1 ){
                  if(diffInMonths > 60){
                    Swal.fire({
                      icon: 'error',
                      text: 'Billing period should be under 5 years under BDA Area'
                    })
                    billingEnd.value = '';
                  }else{
                    let param = {
                      "fromDate": billingStartValue,
                      "toDate": billingEndValue,
                      "intId": sessionStorage.getItem('REGD_ID'),
                      "yearLimit": 5,
                      "onlineProcessId": this.processId,
                      "profileId": this.userId
                  ...
                  if(diffInMonths > 84){
                    Swal.fire({
                      icon: 'error',
                      text: 'Billing period should be under 7 years under BDA Area'
                    })
                    billingEnd.value = '';
                  }else{
                    let param = {
                ...
                Swal.fire({
                  icon: 'error',
                  text: 'Please select billing start date'
                })
              }
            }
          });
      ...
      /*Start of calculation for Marketing Assistance (Unit)*/
     
      case 41:
        this.fromToDateValidation();
        this.validateDate();
        this.fillUpPrevDetails('cls_prevAmount');
        //Adding mendatory sign
        // const internElement = document.querySelector('.cls_intern_d_none');
        // const labelElement = internElement.closest('.dynGridCls').querySelector('label.dynlabel');
        // labelElement.innerHTML = labelElement.textContent + ' <span style="color: red;">*</span>';
        //end of adding mendatory
        let mauParam = {
          "onlineProcessId": this.processId,
          "profileId": this.userId,
          "intId": sessionStorage.getItem('REGD_ID')
        ...
        this.intNoOfDateField = 1;
        // this.fillUpPrevDetails('cls_mauAmount');
      ...
      /*Start of calculation for Marketing Assistance (Association)*/
      case 42:{
        ...
        // let maaEmailId = (<HTMLInputElement>document.getElementsByClassName('cls_miaGstIn')[0]);
        this.loading = true;
      ...
      /*start of calculation for Patent Registration Cost Reimbursement*/
      case 38:
        expense = document.getElementsByClassName('cls_expenses');
        let patParam = {
          "onlineProcessId": this.processId,
        ...
        this.fillUpMdInfo('cls_prcrName', 'cls_prcrDesg');
        //calculation of total expenses
        ...
        let claimDate : any = (<HTMLInputElement>document.getElementsByClassName('cls_date')[0]);
        ...
        this.alphaNumericValidation();
        break;
      /*End of calculation for Patent Registration Cost Reimbursement*/

      /*start of calculation for Incubation Rental Created By Bibhuti bhusan Sahoo Modified By Bindurekha Nayak : 25-06-2024*/
      case 114:
        if (this.formEditStatus == 1) {
          Swal.fire({
            icon: 'warning',
            text: 'Upon Editing, Previous calculation will be changed!',
            confirmButtonText:"Okay"
          }).then((result) => {
            if (result.isConfirmed) {
            this.incubationRentalCalculation();
      ...
      /*start of calculation for Power tariff subsidy*/
      case 22:
        ...
        let ptsUnitName: any = (<HTMLInputElement>document.getElementsByClassName('cls_ptsUnitName')[0]);
        ...
        this.IrmsDetailsService.getRegCompanyDetails(powerParam).subscribe(res => {
          ...
          if(res.status == 200){
            if(res.result.companyDetails){
              ptsCompanyName.value = (ptsCompanyName.value == '') ? res.result.companyDetails.cmpName : ptsCompanyName.value;
              ptsCompanyAddress.value = (ptsCompanyAddress.value == '') ? res.result.companyDetails.cmpAddress : ptsCompanyAddress.value;
              ptsContactAddress.value = (ptsContactAddress.value == '') ? res.result.companyDetails.cmpAddress : ptsContactAddress.value;
            }
            if(res.result.communicationDetails){
              ptsContactName.value = (ptsContactName.value == '') ? res.result.communicationDetails.empName : ptsContactName.value;
              ptsContactDesg.value = (ptsContactDesg.value == '') ? res.result.communicationDetails.empDegn : ptsContactDesg.value;
              ptsContactTel.value = (ptsContactTel.value == '') ? res.result.communicationDetails.empTeleNo : ptsContactTel.value;
              ptsContactMob.value = (ptsContactMob.value == '') ? res.result.communicationDetails.empMobNo : ptsContactMob.value;
              ptsContactEmail.value = (ptsContactEmail.value == '') ? res.result.communicationDetails.empEmail : ptsContactEmail.value;
            }
            if (res.result.unitDetails) {
              ptsUnitName.value = (res.result.unitDetails.unitId !== null && res.result.unitDetails.unitId !== undefined)? res.result.unitDetails.unitId : ptsUnitName.value;
            }
        ...
        ptsTariffClaimed.addEventListener('keyup', () => {
          ...
          if(this.ptsPrevClaimedAmount>0){
            let restAmount = 3500000 - this.ptsPrevClaimedAmount;
          ...
          }else if(totalValue > 3500000){
            ptsIncentiveClaimed.value = '';
            Swal.fire({
              icon: 'error',
              text: 'Incentive amount can not be more than 3500000 /- !'
            });
          }else{
            ptsIncentiveClaimed.value = totalValue;
          }
        ...
        ptsIncentiveClaimed.addEventListener('keyup', () => {
          ...
          if(this.ptsPrevClaimedAmount>0){
            let restAmount = 3500000 - this.ptsPrevClaimedAmount;
            if((totalValue + this.ptsPrevClaimedAmount) > 3500000 ){
              ptsIncentiveClaimed.value = '';
              Swal.fire({
                icon: 'error',
        ...
        this.fillUpMdInfo('cls_ptsMdName', 'cls_ptsDesg');
        this.fromToDateValidation();
        this.alphaNumericValidation();
      break;
      /*End of calculation for Power tariff subsidy*/
      /*start of calculation for Electricity Inspection Fee*/
      case 23:
        ...
        prcrMdElement.value = this.vchMdName;
        prcrMdDesgElement.value = 'Managing Director';
        this.loading = true;
        let prcrParam = {
          "intId": sessionStorage.getItem('REGD_ID')
        };
        this.IrmsDetailsService.getRegCompanyDetails(prcrParam).subscribe(res => {
          ...
          if(res.status == 200){
            if(res.result.companyDetails){
              prcrCompanyName.value = res.result.companyDetails.cmpName;
              prcrCompanyAddress.value = res.result.companyDetails.cmpAddress;
              prcrContactAdress.value = res.result.companyDetails.cmpAddress;
            }
            if(res.result.communicationDetails){
              prcrContactName.value = res.result.communicationDetails.empName;
              prcrContactDesg.value = res.result.communicationDetails.empDegn;
              prcrContactMob.value = res.result.communicationDetails.empMobNo;
              prcrContactEmail.value = res.result.communicationDetails.empEmail;
            }
            if(res.result.dateDetails){
              prcrDateOfIncorporation.value = res.result.dateDetails.incorporationDate;
              prcrDateOfCommercial.value = res.result.dateDetails.commencementDate;
            }
          }
        });
        this.alphaNumericValidation();
      break;
      /*start of calculation for Energy Audit fee Modified by Bindurekha Nayak 08-07-2024*/
      case 28:
        ...
        let leaseUnitName: any = (<HTMLInputElement>document.getElementsByClassName('cls_unitType')[0]);
      ...
      /*Start of calculation for Cost of Cleaner / Greener Production Measuree */
      case 59:{
        this.alphaNumericValidation();
        ...
        this.IrmsDetailsService.getRegCompanyDetails(cocParam).subscribe(res => {
          ...
          if(res.status == 200){
            if(res.result.unitDetails){
              cocTotalEnergy.value = (cocTotalEnergy.value == '') ? res.result.unitDetails.unitType : cocTotalEnergy.value;
             }
          }
        });
      ...
      /*Start of calculation for Human Capital Investment Subsidy New Functionality add by Bindurekha Nayak date: 01-07-2024*/
      case 60:{
        let hcisParam = {
          "onlineProcessId": this.processId,
          "profileId": this.userId,
        ...
        this.IrmsDetailsService.getPastRecordEmpTotal(hcisParam).subscribe(res => {
          if(res.status == 200){
            this.previousClaimedSubsidy = res.result;
            this.booleanPrevClaimStatus = ((res.result).length) ? true : false;
            let totalAmount = 0;
        ...
        this.fillUpPrevDetails('cls_hcisPrevClaimAmount');
        this.fillUpMdInfo('cls_hcisName', 'cls_hcisDegn');
        this.fromToDateValidation();
      
        let totalHumanCapInvElement: any = document.getElementsByClassName('cls_totalHumanCapInv')[0];
        let hcisAmount: any = document.getElementsByClassName('cls_hcisAmount')[0];
        let hcFromDate: any = document.getElementsByClassName('cls_hcFromDate')[0];
        let hcToDate: any = document.getElementsByClassName('cls_hcToDate')[0];
        let hcNoOfEmployee: any = document.getElementsByClassName('cls_hcisEmp')[0];
        let hcperEmpAmount: any = document.getElementsByClassName('cls_perEmpAmount')[0];
        let claimPeriodElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_claimPeriod')[0]);
       let previousClaimPeriod = parseInt(claimPeriodElement.value);
          claimPeriodElement.addEventListener('change', function () {
              let claimPeriod = parseInt(claimPeriodElement.value);
         ...
         hcFromDate.addEventListener('change', () => {
          let claimPeriod = parseInt(claimPeriodElement.value);
          if(claimPeriod <= 0){
            Swal.fire({
              icon: 'error',
          ...
          let startDate = new Date(hcFromDate.value);
          let endDate = new Date(hcToDate.value);
          if (endDate < startDate) {
            hcToDate.value = '';
             Swal.fire({
               icon: 'error',
             ...
             this.IrmsDetailsService.getyearCheckIncubationRental(prvParam).subscribe(res => {
               ...
               if (res.status == 2) {
                 ...
                 if (claimPeriod > ClaimablePeriod) {
                   ...
                   let fromDate = new Date(hcFromDate.value);
                   let toDate = new Date(fromDate);
                   let currentDate = new Date();
                   toDate.setFullYear(fromDate.getFullYear() + claimPeriod);
                   // Validating if toDate is less than or equal to current date
                   if (toDate <= currentDate) {
                     let formattedToDate = toDate.toISOString().slice(0, 10);
                     hcToDate.value = formattedToDate;
                     hcToDate.readOnly = true;
                     hcToDate.dispatchEvent(new Event('change'));
                   } else {
                     Swal.fire({
                       icon: 'error',
                       text: 'As of current date, you are not eligible for ' + (claimPeriod > 1 ? claimPeriod + ' years' : claimPeriod + ' year') + ' of subsidy'
        ...
        hcToDate.readOnly = true;
        hcToDate.addEventListener('change',  () => {
          hcToDate.readOnly = true;
          let startDate = new Date(hcFromDate.value);
          let endDate = new Date(hcToDate.value);
          if (endDate < startDate) {
            ...
            let policyEndDateCheck = this.eligibilityEndDateCheck('cls_hcToDate ');
            if(policyEndDateCheck==0){
              let startDate = new Date(hcFromDate.value);
              let endDate = new Date(hcToDate.value);
        ...
        hcisAmount.addEventListener('input', () => {
          let hcTotalAmount = hcisAmount.value / 2;
          let isCappingValueDet = (this.totalPreviousAmount > 0) ? 2000000 - this.totalPreviousAmount : 2000000;
          this.isCappingValueDets=isCappingValueDet;
          if(this.hcEligibleAmount>0){
            totalHumanCapInvElement.value = (hcTotalAmount>isCappingValueDet) ? isCappingValueDet : hcTotalAmount;
            if (parseInt(totalHumanCapInvElement.value) > isCappingValueDet) {
              Swal.fire({
                icon: 'error',
                text: 'Subsidy claimed amount cannot be more than ' + isCappingValueDet
              });
              totalHumanCapInvElement.value = isCappingValueDet;
            }
          }else{
            totalHumanCapInvElement.value =0;
          }
        });
        totalHumanCapInvElement.addEventListener('input', () => {
          let value = parseFloat(totalHumanCapInvElement.value);
          if (!isNaN(value)) {
            totalHumanCapInvElement.value = Math.min(value, this.isCappingValueDets);
          }
        });
        this.alphaNumericValidation();
      }
      break;
      /*End of calculation for Human Capital Investment Subsidy */
      /*Start of calculation for EPF and ESI */
      case 35:{
        ...
        this.alphaNumericValidation();                         //to validate special characters
        ...
        let eeBdaCapingValue: any       = 50000000;
        let eeNonBdaCapingValue: any    = 70000000;
        this.intNoOfDateField           = 2;
        //removing 4 & 5 year option
        eeSubsidyPeriodElement.remove(eeSubsidyPeriodElement.options.length - 1);
        eeSubsidyPeriodElement.remove(eeSubsidyPeriodElement.options.length - 1);
        ...
        fromDateElement.addEventListener('change', () => {
          ...
          let subsidyPeriod = parseInt(eeSubsidyPeriodElement.value) || 0;
              ...
              if(eeBdaValue == "" || eeBdaValue == undefined || eeBdaValue == 0){
                ...
                this.IrmsDetailsService.getElecEpfAppliedYears(rdParam).subscribe(res => {
                  if(res.status == 200){
                    ...
                    let applicableSubsidyPeriodCal = (subsidyPeriod <= applicableSubsidyPeriod)? subsidyPeriod : applicableSubsidyPeriod; 
                    if(eeBdaValue == 1) {
                      if(applicableSubsidyPeriod <= 0 ) {
                        ...
                        fromDateElement.value        = '';
                        toDateElement.value          = '';
                        eeBdaCapingValue             = eeNonBdaCapingValue = 0;

                      } else {
                        eeBdaCapingValue  = 50000000 * applicableSubsidyPeriodCal;
                      }
                    } else if(eeBdaValue == 2) {
                      eeNonBdaCapingValue = 70000000 * applicableSubsidyPeriodCal;
                    } else {
                      eeBdaCapingValue    = 50000000;
                      eeNonBdaCapingValue = 70000000;
                    }
                  } else {
                    eeBdaCapingValue      = 50000000 * subsidyPeriod;
                    eeNonBdaCapingValue   = 70000000 * subsidyPeriod;
                  }
                });
              }
          } else {
      ...
      eeAmountElementDet.addEventListener('input', () => {
          let eeAmountElementVal = String(eeAmountElementDet.value);
          console.log(eeBdaValue);
          if(eeBdaValue == "" || eeBdaValue == undefined || eeBdaValue == 0){
            eeBdaTotalElement.value = '';
            Swal.fire({
              icon: 'error',
            ...
            eeBdaTotalElement.value = (eeAmountElementVal <= eeBdaCapingValue) ?  eeAmountElementVal : eeBdaCapingValue;
          } else if (eeBdaValue == 2) {
            eeBdaTotalElement.value = (eeAmountElementVal <= eeNonBdaCapingValue) ?  eeAmountElementVal : eeNonBdaCapingValue;
          }
        });
        eeBdaTotalElement.addEventListener('input', () => {
          let eeBdaTotalElementVal = parseInt(eeBdaTotalElement.value);
          if(eeBdaValue == "" || eeBdaValue == undefined || eeBdaValue == 0){
            eeBdaTotalElement.value = '';
            Swal.fire({
              icon: 'error',
            ...
            eeBdaTotalElement.value = (eeBdaTotalElementVal <= eeBdaCapingValue) ?  eeBdaTotalElementVal : eeBdaCapingValue;
          } else if (eeBdaValue == 2) {
            eeBdaTotalElement.value = (eeBdaTotalElementVal <= eeNonBdaCapingValue) ?  eeBdaTotalElementVal : eeNonBdaCapingValue;
          }
        });
      }
      break;
      ...
      /*Start of calculation for R&D Incentive */
      case 62:
        let rdParam = {
          "onlineProcessId": this.processId,
          "profileId": this.userId,
        ...
        this.intNoOfDateField = 2;
        this.fillUpPrevDetails('cls_rdAmount');
        ...
        let finalCappingAmount:any = 10000000;
        ...
        rdTotalExpenseAmount.addEventListener('input', () => {
          totalRdAmount = String(rdTotalExpenseAmount.value);
          let claimAmount = String((50/100) * totalRdAmount);
          finalClaimAmount = (claimAmount <= finalCappingAmount) ? claimAmount : finalCappingAmount;
          rdFinalClaimAmount.value = finalClaimAmount;
        });
        rdFinalClaimAmount.addEventListener('input', () => {
          let amount = String(rdFinalClaimAmount.value);
          finalClaimAmount = (amount <= finalCappingAmount) ? amount : finalCappingAmount;
          rdFinalClaimAmount.value = finalClaimAmount;
        });
      ...
      /*Start of calculation for Internet Connectivity Incentive */
      case 64: {
        let rdParam = {
          "onlineProcessId": this.processId,
          "profileId": this.userId,
        ...
        this.intNoOfDateField = 2;
        this.validateDate();
        this.fillUpMdInfo('cls_iciName', 'cls_rfinDesn');
        this.fillUpPrevDetails('cls_rfinAmount');
        this.fromToDateValidation();
        this.alphaNumericValidation();

        let fromDate: HTMLInputElement = <HTMLInputElement>document.getElementsByClassName('cls_expense_from')[0];
        let toDate: HTMLInputElement = <HTMLInputElement>document.getElementsByClassName('cls_expense_to')[0];
        let iciAmountEle: HTMLInputElement = <HTMLInputElement>document.getElementsByClassName('cls_iciExpense')[0];
        let iciTotalAmountEle: HTMLInputElement = <HTMLInputElement>document.getElementsByClassName('cls_rfinTotalIntConn')[0];
        let claimPeriodElement: HTMLInputElement = <HTMLInputElement>document.getElementsByClassName('cls_claimPeriod')[0];
        let claimPeriodValue = parseInt(claimPeriodElement.value) || 0;
        let sessionInfo = JSON.parse(sessionStorage.getItem('FFS_SESSION'));
        let isFirstChangeEvent : Boolean = true;
        iciTotalAmountEle.readOnly = true;

        claimPeriodElement.addEventListener('change', () => {
        ...
        fromDate.addEventListener('change', () => {
          if(!(claimPeriodElement.value) || parseInt(claimPeriodElement.value) == 0) {
            Swal.fire({
              icon: 'error',
              text: 'Please select Claim Period!'
            });
            fromDate.value = '';
            return;
          } else {
            claimPeriodValue  = parseInt(claimPeriodElement.value) || 0;
            let fromDateValue = new Date(fromDate.value);
            let toDateValue   = new Date(fromDateValue);
            let currentDate   = new Date();
            toDateValue.setFullYear(fromDateValue.getFullYear() + claimPeriodValue);
            // Validating if toDate is less than or equal to current date
            if (toDateValue <= currentDate) {
              let formattedToDate    = toDateValue.toISOString().slice(0, 10);
              toDate.value           = formattedToDate;
              toDate.readOnly        = true;
              //start of showing policy commencement end notification
              let policyEndDateCheck = this.eligibilityEndDateCheck('cls_isPisClaimTo');
              if(policyEndDateCheck  == 0){
              let startDateValue   = new Date(fromDate.value);
              let endDateValue     = new Date(toDate.value);
              let diffMonth        = (endDateValue.getFullYear() - startDateValue.getFullYear()) * 12 + (endDateValue.getMonth() - startDateValue.getMonth());
              if (diffMonth < 0) {
                diffMonth = 0;
              }
              } else {
              ...
              //end of showing policy commencement end notification
            } else { 
              Swal.fire({
                icon: 'error',
                text: 'As of current date, you are not eligible for ' + (claimPeriodValue > 1 ? claimPeriodValue + ' years' : claimPeriodValue + ' year') + ' of subsidy'
              })
              fromDate.value = '';
              toDate.value = '';
              return;
        ...
        // Creating button After Final Amount Field
        const parentDiv = iciTotalAmountEle.closest('.dynGridCls');
        const labelElement = parentDiv.querySelector('.dynlabel');
        let button = document.createElement('button');
        button.textContent = 'Calculate Incentive';
        button.classList.add('btn', 'btn-outline-primary', 'mb-2', 'mx-2', 'incCalculateBtn');
        button.addEventListener('click', () => {
          calculateIncentive(fromDate, toDate, iciAmountEle, iciTotalAmountEle, sessionInfo);
        });
        labelElement.insertAdjacentElement('afterend', button);

        // Define the arrow function inside the case block
        const calculateIncentive = (fromDate: HTMLInputElement, toDate: HTMLInputElement, iciAmountEle: HTMLInputElement, iciTotalAmountEle: HTMLInputElement, sessionInfo: any) => {
          claimPeriodValue  = parseInt(claimPeriodElement.value) || 0;
          //show alert message if fromDate or iciAmountEle is empty
          if (!fromDate.value || !iciAmountEle.value) {
            Swal.fire({
              icon: 'error',
              text: 'Please fill Amount and Date to calculate Incentive!'
            });
            return;
          }
          ...
          if(claimPeriodValue > ClaimablePeriod){
            Swal.fire({
              icon: 'error',
              text: 'You Can apply maximum ' + ClaimablePeriod + ' year!'
            });
            ...
            claimPeriodValue = parseInt(claimPeriodElement.value) || 0;
            let iciAmountEleValue = parseInt(iciAmountEle.value) || 0;
            if(claimPeriodValue > 0 && iciAmountEleValue > 0){
              let CappingAmount = 500000 * ClaimablePeriod;
              let incentiveAmount = 0.5 * iciAmountEleValue;
              iciTotalAmountEle.value = (incentiveAmount > CappingAmount ? CappingAmount : incentiveAmount).toString();
            }
          }
            }
          })
      ...
      break;
      /*End of calculation for Internet Connectivity Incentive */

      /*Start of calculation for VFX and Animation */
      case 63:
        this.fillUpMdInfo('cls_vfxName', 'cls_vfxDesn');
        this.IrmsDetailsService.getCompanyVfxDetails(vfxParam).subscribe(res => {
          this.loading = false;
          if(res.status == 200){
              // console.log(res);
          }
        ...
        this.alphaNumericValidation();
      break;
      /*End of calculation for VFX and Animation */
      /*Start of calculation for IT Park */
      case 53:
        this.fillUpMdInfo('cls_itName', 'cls_itDesg');
        this.alphaNumericValidation();
      break;
      /*End of calculation for IT Park */
      /*Start of calculation for Co-Working Space Rent Reimbursement  Modified by Bindurekha Nayak 08-07-2024*/
      case 54:
        //this.checkFieldAmount('cls_cwsRent','cls_cwsTotalSubsidy');
        let cwsSeatAccEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsSeatAcquired')[0]);
        let cwsRatePerSeatEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsRatePerSeat')[0]);
        let cwsMonthlyPayEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsMonthlyRent')[0]);
        let cwsStartDateEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsStartDate')[0]);
        let cwsEndDateEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsEndDate')[0]);
        let cwsRentPaidEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsRent')[0]);
        let cwsTotalSubsidyEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsTotalSubsidy')[0]);
        let cwsControlForCal:any = document.getElementsByClassName('cls_cwsCal');
        let cwsGovtEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_cwsGovtOwned')[0]);
        let govtOwnedCheckBoxes: any = document.querySelectorAll('.cls_cwsGovtOwned');
        
        cwsEndDateEle.addEventListener('input', () =>{
          let startDate = cwsStartDateEle.value;
          let endDate = cwsEndDateEle.value;
          let fromDate = new Date(startDate);
          let toDate = new Date(endDate);
          let difference = toDate.getTime() - fromDate.getTime();
          let diffMon = Math.floor(difference / (1000 * 60 * 60 * 24 * 30.44));
          ...
          if(toDate < fromDate){
           ...
           govtOwnedCheckBoxes.forEach(radioButton => {
              radioButton.addEventListener('click', () => {
                  this.isGovtOwnedDet = parseInt(radioButton.value); 
                  let previousDet = parseInt(this.isGovtOwnedDet);
                  if (this.isGovtOwnedDet !== previousDet) {
                      previousDet = this.isGovtOwnedDet;
                      
                      if (this.isGovtOwnedDet <= 0) { 
                          cwsTotalSubsidyEle.value = '';
                          cwsStartDateEle.value = '';
                          cwsEndDateEle.value = '';
                          cwsSeatAccEle.value = '';
                          cwsMonthlyPayEle.value = '';
                          cwsRatePerSeatEle.value = '';
                          cwsRentPaidEle.value = '';
                          
                           Swal.fire({
                              icon: 'error',
                              text: 'Please select Government Owned Facility!'
                          });
                      } else {
                           cwsTotalSubsidyEle.value = '';
                          cwsStartDateEle.value = '';
        ...
        let cwsSeatAccVal = 0;
        let cwsMonthlyPayVal = 0;
        let diffMonth = 0
        let totalPayValue = 0
        let perSeatValuePerMon = 0;
        for (let con of cwsControlForCal){
          con.addEventListener('input', () =>{
            cwsSeatAccVal = isNaN(cwsSeatAccEle.value) ? 0 : cwsSeatAccEle.value;
            cwsMonthlyPayVal = isNaN(cwsMonthlyPayEle.value) ? 0 : cwsMonthlyPayEle.value;
            let startDate = new Date(cwsStartDateEle.value);
            let endDate = new Date(cwsEndDateEle.value);
            ...
            let diffMonth = (endYear - startYear) * 12 + (endMonth - startMonth);
            //let difference = endDate.getTime() - startDate.getTime();
           // diffMonth = isNaN(Math.floor(difference / (1000 * 60 * 60 * 24 * 30.44))) ? 0 : Math.floor(difference / (1000 * 60 * 60 * 24 * 30.44));
            if(diffMonth < 0){
              cwsEndDateEle.value = '';
              diffMonth = 0;
              Swal.fire({
                icon: 'error',
                text: 'Please Enter correct end date !'
              });
            }else{
              if (!isNaN(cwsMonthlyPayVal) && !isNaN(diffMonth)) {
                totalPayValue = cwsMonthlyPayVal * diffMonth;
                cwsRentPaidEle.value = totalPayValue;
            }
              perSeatValuePerMon = 0;
              if(diffMonth > 0 && cwsMonthlyPayVal > 0 && cwsSeatAccVal > 0){
                perSeatValuePerMon = cwsMonthlyPayVal/cwsSeatAccVal;
              }
              if(this.isGovtOwnedDet == 1 || this.isGovtOwnedDet == 2){
                if(this.isGovtOwnedDet == 1 && diffMonth > 0){
                  if(perSeatValuePerMon > 1400){
                    cwsTotalSubsidyEle.value = 1400 * cwsSeatAccVal * diffMonth;
                  }else{
                    cwsTotalSubsidyEle.value = totalPayValue;
                  }
                }else if(this.isGovtOwnedDet == 2 && diffMonth > 0){
                  if(perSeatValuePerMon > 1000){
                    cwsTotalSubsidyEle.value = 1000 * cwsSeatAccVal * diffMonth;
                  }else{
                    cwsTotalSubsidyEle.value = totalPayValue;
                  }
                }
              }else if(this.isGovtOwnedDet ==''){
                Swal.fire({
                  icon: 'error',
                  text: 'Please Select the Govt owned facility !'
                });
                cwsTotalSubsidyEle.value='';
                cwsStartDateEle.value='';
                cwsEndDateEle.value='';
        ...
        // cwsGovtEle.addEventListener('change', ()=>{
        //   if(isGovtOwned == 1 || isGovtOwned == 2){
        //     if(isGovtOwned== 1 && diffMonth > 0){
        //       if(perSeatValuePerMon > 1400){
        //         cwsTotalSubsidyEle.value = 1400 * cwsSeatAccVal * diffMonth;
        //       }else{
        //         cwsTotalSubsidyEle.value = totalPayValue;
        //       }
        //     }else if(isGovtOwned == 2 && diffMonth > 0){
        //       if(perSeatValuePerMon > 1000){
        //         cwsTotalSubsidyEle.value = 1000 * cwsSeatAccVal * diffMonth;
        //       }else{
        //         cwsTotalSubsidyEle.value = totalPayValue;
        ...
        //   }
        // })
        cwsStartDateEle.addEventListener('input', ()=>{
          cwsEndDateEle.value="";
          cwsRentPaidEle.value='';
          cwsTotalSubsidyEle.value='';
        });
        cwsMonthlyPayEle.addEventListener('input', ()=>{
          cwsMonthlyPayEle.value = cwsSeatAccEle*cwsRatePerSeatEle;
          cwsStartDateEle.value='';
          cwsEndDateEle.value='';
          cwsRentPaidEle.value='';
          cwsTotalSubsidyEle.value='';
        ...
        cwsRatePerSeatEle.addEventListener('input', ()=>{
          cwsStartDateEle.value='';
          cwsEndDateEle.value='';
          cwsRentPaidEle.value='';
          cwsTotalSubsidyEle.value='';
        });
        this.fillUpMdInfo('cls_cwsName', 'cls_cwsDesg');
        this.fromToDateValidation();
        this.alphaNumericValidation();
      break;
      /*End of calculation for Co-Working Space Rent Reimbursement */
      /*Start of calculation for Conversion Charges */
      case 55:
        this.checkFieldAmount('cls_paid_amount','cls_subsidy_amount');
        this.fillUpMdInfo('cls_ccName', 'cls_ccDesg');
        this.alphaNumericValidation();
      break;
      /*End of calculation for Conversion Charges */
      /*Start of calculation for Biju Innovation Lab (BIL) developed by Bindurekha Nayak 28-02-2024*/
      case 65:
        this.fillUpMdInfo('cls_bilName', 'cls_bilDesg');
         let totalClaimedAmount : any = (<HTMLInputElement>document.getElementsByClassName('cls_subsidy')[0]);
         expense = document.getElementsByClassName('cls_expense');
         let bijuCappingVal:any = 2000000;
          for (let button of expense) {
            button.addEventListener("input", () => {
              let total = 0;
              for (let i = 0; i < expense.length; i++) {
                let data = expense[i].value;
                if (!isNaN(parseFloat(data))) {
                  total += parseFloat(data);
                }
              }
              totalClaimedAmount.value = String(total);
              if(totalClaimedAmount.value >= 2000000){
                Swal.fire({
                  icon: 'error',
                  text: 'Subsidy claimed amount can not be more than 2000000/-'
                });
                totalClaimedAmount.value = 0;
                expense.value=0;
              }
            });
          }
          totalClaimedAmount.addEventListener('input', () => {
            if(totalClaimedAmount.value >= 2000000){
              Swal.fire({
                icon: 'error',
                text: 'Subsidy claimed amount can not be more that 2000000/-'
              });
              totalClaimedAmount.value = 0;
              expense.value=0;
            }
        ...
        this.alphaNumericValidation();
      break;
      /*End of calculation for Biju Innovation Lab (BIL) */
      /*Start of calculation for Registration Charges */
      case 56:
        this.checkFieldAmount('cls_fees_paid','cls_subsidy');
        this.fillUpMdInfo('cls_rcName', 'cls_rcDesg');
        this.alphaNumericValidation();
      break;
      /*End of calculation for Registration Charges */
      /*Start of calculation for Center of Excellence Incentive developed by Bindurekha Nayak 28-02-2024*/
      case 66:
         this.fillUpMdInfo('cls_ceiName', 'cls_ceiDesg');
         this.alphaNumericValidation();
      break;
      /*End of calculation for Center of Excellence Incentive */
      /*Start of calculation for Emerging Technology Incentive developed by Bindurekha Nayak 28-02-2024*/
      case 67:
         this.fillUpMdInfo('cls_etiName', 'cls_etiDesg');
         this.alphaNumericValidation();
      break;
      /*End of calculation for Emerging Technology Incentive */
      /*Start of calculation for Skill Upgradation Incentive */
      case 61:{
        this.alphaNumericValidation();
        let suiParam = {
          "onlineProcessId": this.processId,
        ...
        this.IrmsDetailsService.getPastRecordEmpTotal(suiParam).subscribe(res => {
          if(res.status == 200){
            this.previousClaimedSubsidy = res.result;
            this.booleanPrevClaimStatus = ((res.result).length) ? true : false;
          }
        ...
        this.intNoOfDateField = 2;
        this.fillUpPrevDetails('cls_sui_amount');
        let skillFromDate: any = (<HTMLInputElement>document.getElementsByClassName('cls_skillFromDate')[0]);
        let skillToDate: any = (<HTMLInputElement>document.getElementsByClassName('cls_skillToDate')[0]);
        let subsidyClaimedEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_suiIncentive')[0]);
        let totalIncentiveElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_suiTotal')[0]);
       
        subsidyClaimedEle.addEventListener('input', () => {
          if(claimPeriodElement.value>0){
          let totalSkillAmount = (claimPeriodElement.value)*150000;
          let subsidyClaimedVal = isNaN(subsidyClaimedEle.value) ? 0 : subsidyClaimedEle.value;
          totalIncentiveElement.value = (subsidyClaimedVal > totalSkillAmount) ? totalSkillAmount : subsidyClaimedVal;
          }
        });
        totalIncentiveElement.addEventListener('input', () => {
          if(claimPeriodElement.value>0){
            let totalSkillAmount = (claimPeriodElement.value)*150000;
            let subsidyClaimedVal = isNaN(subsidyClaimedEle.value) ? 0 : subsidyClaimedEle.value;
            totalIncentiveElement.value = (subsidyClaimedVal > totalSkillAmount) ? totalSkillAmount : subsidyClaimedVal;
        ...
        let previousClaimPeriod = parseInt(claimPeriodElement.value);
          claimPeriodElement.addEventListener('change', function () {
              ...
              if (claimPeriod !== previousClaimPeriod) {
                  ...
                  if (claimPeriod <= 0) {
                      ...
                      skillFromDate .value = '';
                      skillToDate.value = '';
                      subsidyClaimedEle.value ='';
                      totalIncentiveElement.value='';
                  } else {
                    skillFromDate.value = '';
                    skillToDate.value = '';
                    subsidyClaimedEle.value ='';
          ...
          skillFromDate.addEventListener('change', () => {
            let claimPeriod = parseInt(claimPeriodElement.value);
            if(claimPeriod <= 0){
              Swal.fire({
                icon: 'error',
            ...
            let startDate = new Date(skillFromDate.value);
            let endDate = new Date(skillToDate.value);
            if (endDate < startDate) {
                skillToDate.value = '';
                Swal.fire({
                  icon: 'error',
                ...
                let prvParam = {
                  ...
                  "onlineProcessId": this.processId,
                  "profileId": sessionInfo.USER_ID,
                  "incStartDate": skillFromDate.value,
                  "claimLimit": 5,
                };
                this.loading = true;
                this.IrmsDetailsService.getyearCheckIncubationRental(prvParam).subscribe(res => {
                  ...
                  if (res.status == 2) {
                    ...
                    if (claimPeriod > ClaimablePeriod) {
                      ...
                      let fromDate = new Date(skillFromDate.value);
                      let toDate = new Date(fromDate);
                      let currentDate = new Date();
                      toDate.setFullYear(fromDate.getFullYear() + claimPeriod);
                      // Validating if toDate is less than or equal to current date
                      if (toDate <= currentDate) {
                        let formattedToDate = toDate.toISOString().slice(0, 10);
                        skillToDate.value = formattedToDate;
                        skillToDate.readOnly = true;
                        skillToDate.dispatchEvent(new Event('change'));
                      } else {
                        Swal.fire({
                          icon: 'error',
                          text: 'As of current date, you are not eligible for ' + (claimPeriod > 1 ? claimPeriod + ' years' : claimPeriod + ' year') + ' of subsidy'
          ...
          skillToDate.readOnly = true;
          skillToDate.addEventListener('change',  () => {
              let startDate = new Date(skillFromDate.value);
              let endDate = new Date(skillToDate.value);
              if (endDate < startDate) {
                skillToDate.value = '';
        ...
        this.fillUpMdInfo('cls_skiName', 'cls_skiDesg');
        this.fromToDateValidation();
      }
      break;
      /*End of calculation for Skill Upgradation Incentive */
      /*Start of calculation for Intern Stipend Reimbursement Modified by Bindurekha Nayak 08-07-2024*/
      case 37:
        ...
        this.intNoOfDateField = 2;
        this.fillUpPrevDetailsNew('cls_prev_claim_hd');
   ...
   if (Number(totalInternEle.value) < Number(sdteEle.value)) {
      ...
      /*Start of calculation for Recruitment Assistance IT Policy */
      case 69:
...
totalTabularEditForm(addMoreId:any)
 ...
 storeAddMoreMergeDetails(formctrls:any,addMoreId:any)
  ...
  addMoreCalculation(data, ctrlId, formName){
    ...
    if(this.processId == 68){
      ...
      this.invSubCapitalAmount();
    }else if(this.processId == 60){
      data[ctrlId].forEach(element => {
        if (element[1].ctrlValue !== '' && element[3].ctrlValue !== '') {
            let totalCourseFeeValue = element[3].ctrlValue < 10000 ? element[3].ctrlValue : 10000;
            let addMoreInputValue = parseInt(element[1].ctrlValue) * parseInt(totalCourseFeeValue);
            totalAmount += addMoreInputValue;
        }
    });
    
    this.hcEligibleAmount = totalAmount;
    this.humanCapitalAmount();
   }else if(this.intProductId == 111){
    this.skillUpgradeAddmoreCount--;
   }
  }
  /**
   * Function Name: interestSubsidyAddMoreCalculation
   * Description: It calculate the add more field given field value
   * Created By: Bibhuti Bhusan Sahoo
   * Created Date: 05th Jan 2024
   */
  interestSubsidyAddMoreCalculation(data, ctrlId, formName){
    let totalAmount = 0;
      ...
      this.totalRepaymentInterAmount = totalAmount;
  ...
  /**
   * Function Name: humanCapitalAmount
   * Description: It calculate the total capital amount for "Human capital Form"
   * Created By: Bindurekha Nayak
   * Created Date: 01 July 2024  */
  humanCapitalAmount(){
    let totalAmount = this.hcEligibleAmount;
    let hcisAmount: any = (<HTMLInputElement>document.getElementsByClassName('cls_hcisAmount')[0]);
    hcisAmount.value = totalAmount;
    let event = new Event("input");
    hcisAmount.dispatchEvent(event)
  }
  /**
   * Function Name: calculateTotalRepayment
   * Description: It calculate the total capital amount for "Interest Subsidy Form"
   * Created By: Bibhuti Bhusan Sahoo
   * Created Date: 05th Jan 2024
   */
  calculateTotalRepayment(){
    let repaymentElement: any = (<HTMLInputElement>document.getElementsByClassName('cls_isTotRepayment')[0]);
    let totalRepaymentelement: any = (<HTMLInputElement>document.getElementsByClassName('cls_isPisClaimTotal')[0]);
    repaymentElement.value = this.totalRepaymentInterAmount;
    let event = new Event("input");
    repaymentElement.dispatchEvent(event);
    // let percentageAmountRepayment = this.totalRepaymentInterAmount * 0.05;
    // totalRepaymentelement.value = (percentageAmountRepayment > 0) ? percentageAmountRepayment : 0;
  ...
  getPrevClaimDetails(intId: any) {
    ...
    if(this.processId == 60){
      $('.cls_hcisPrevClaimAmount').val(this.previousClaimedSubsidy[index].total_int_prev_cia_amt);
      $('.cls_hcprevFromDate').val(this.previousClaimedSubsidy[index].dtmCreatedOn);
      $('.cls_hcprevToDate').val(this.previousClaimedSubsidy[index].stmUpdatedOn);
      this.keyUpEvent('cls_hcisPrevClaimAmount');
      this.modalService.dismissAll();
      ...
      this.modalService.dismissAll();
    }else if(this.processId == 61){
      $('.cls_sui_amount').val(this.previousClaimedSubsidy[index].total_int_prev_cia_amt);
      this.keyUpEvent('cls_sui_amount');
      $('.cls_sui_fromDate').val(this.previousClaimedSubsidy[index].dtmCreatedOn);
      $('.cls_sui_toDate').val(this.previousClaimedSubsidy[index].stmUpdatedOn);
      this.modalService.dismissAll();
    }else if(this.processId == 38){
  ...
  fillUpPrevDetailsNew(eleClass:any){
    let ClaimEvent: any = (<HTMLInputElement>document.getElementsByClassName(eleClass)[0]);
    let button = document.createElement('button');
    button.textContent = 'Select Previous Claimed Details';
    button.classList.add('btn','btn-outline-primary','mb-2','custom-dynamic-button');
  ...
  */
  checkFieldAmount(cls_fisrstField, cls_secondField){
    const firstField = (<HTMLInputElement>document.getElementsByClassName(cls_fisrstField)[0]);
    const secondField = (<HTMLInputElement>document.getElementsByClassName(cls_secondField)[0]);
      ...
      secondField.addEventListener('change', () => {
        if (firstField.value) {
          if ((parseFloat(firstField.value) ) < (parseFloat(secondField.value))) {
            ...
            Swal.fire({
              icon: 'error',
              html: `${secondFieldLabel}can't be greater then ${firstFieldLabel}`
            });
          }
        } else {
          secondField.value = '';
          Swal.fire({
            icon: 'error',
            html: `please first enter the value of ${firstFieldLabel}`
          })
        }
      });
...
getFinancialYearWithQuarters(startDate: Date, endDate: Date): FinancialYear[] {
  ...
  internetConDateValidation(incfromDate, inctoDate){
    let fromDate = new Date(incfromDate.value);
    let toDate = new Date(inctoDate.value);

    // let difference = toDate.getTime() - fromDate.getTime();
    // let diffMon = Math.floor(difference / (1000 * 60 * 60 * 24 * 30.44));
    // let yearDifference = diffMon / 12;
    let yearDifference = toDate.getFullYear() - fromDate.getFullYear();
    let monthDifference = (yearDifference * 12) + (toDate.getMonth() - fromDate.getMonth());
    if(toDate < fromDate){
      inctoDate.value = '';
      Swal.fire({
        icon: 'error',
    ...
    }else if(yearDifference > 2 && monthDifference > 24){
      inctoDate.value = '';
      Swal.fire({
        icon: 'error',
        text: 'Can not claim more than 2 Years!'
      });
    }
    ...
    //   Swal.fire({
    //     icon: 'error',
    //     text: 'You can choose a maximum of 1 year!'
    //   });
    // }
    else if (yearDifference == 1 || ((yearDifference > 1 && monthDifference > 12) && (yearDifference < 2 && monthDifference < 24) )) {
      let iciTotalAmountEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_rfinTotalIntConn')[0]);
        ...
        let iciCappingVal:any = 500000;
        iciAmountEle.addEventListener('input', () => {
          iciAmountVal = String(iciAmountEle.value);
          iciAmountVal = iciAmountVal * 0.50;
    ...
    }else if (yearDifference == 2 || (yearDifference > 2 &&  monthDifference > 24)) {
      let iciTotalAmountEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_rfinTotalIntConn')[0]);
      let iciAmountEle: any = (<HTMLInputElement>document.getElementsByClassName('cls_iciExpense')[0]);
      let iciAmountVal:any = 0;
      let iciTotalAmountVal:any = 0;
      let iciCappingVal:any = 500000*2;
      iciAmountEle.addEventListener('input', () => {
        iciAmountVal = String(iciAmountEle.value);
        iciAmountVal = iciAmountVal * 0.50;
        iciTotalAmountVal = (iciAmountVal <= iciCappingVal) ? iciAmountVal : iciCappingVal;
  ...
  incubationRentalCalculation() {
    let  compSession   = sessionStorage.getItem('companyDetails');
    let companyDetails = JSON.parse(compSession);
    ...
    let intTotalEmpCount: any = companyDetails[0].int_total_emp ? parseInt(companyDetails[0].int_total_emp) : 0;  //changed as suggested by kalyan sir.
    leaseArea.addEventListener('change', function () {
      feePaidAmount.value='';
      leaseStartDate.value='';
      leaseEndDate.value='';
      totalRentalAmount.value='';
      if(isGovtOwned == ''){
        ...
        Swal.fire({
          icon: 'error',
          text: 'Please choose govt owned facility option!'
        });
      }else if(isGovtOwned == 1){
        let area = leaseArea.value;
        if(intTotalEmpCount < 10){
          leaseArea.value = '';
          Swal.fire({
            icon: 'error',
            text: 'Minimum employee strength should not be 10!'
          });
        }else if(intTotalEmpCount >= 10 && intTotalEmpCount < 20){
          if(area > 300){
            leaseArea.value = '';
            Swal.fire({
              icon: 'success',
              text: 'As per your employee strength, you are eligible for incentive upto 300 square feet'
            });
          }
        }else if(intTotalEmpCount >= 20 && intTotalEmpCount < 40){
          if(!(area > 300 && area <= 600)){
            leaseArea.value = '';
            Swal.fire({
              icon: 'error',
              text: 'As per your employee strength, Lease area should be within 301-600 square feet'
            });
          }
        }else if(intTotalEmpCount >= 40){
          if(!(area > 600 && area <= 1000)){
            leaseArea.value = '';
            Swal.fire({
              icon: 'error',
              text: 'As per your employee strength, Lease area should be within 601-1000 square feet'
            });
          }
        }
      }
    ...
    leaseStartDate.addEventListener('change', () => {
      ...
      }else if(isGovtOwned == 1 || isGovtOwned == 2){
    ...
    feePaidAmount.addEventListener('input', function() {
      if(isGovtOwned == ''){
        ...
        if(feePaidAmount.value > 0){
          if(intTotalEmpCount >= 10 && intTotalEmpCount < 20){
      ...
      }else if(isGovtOwned == 2){
        if(leaseArea.value == ''){
          feePaidAmount.value = '';
          Swal.fire({
            icon: 'error',
            text: 'Please enter lease area value!'
          });
        }else{
          let areawiseValue = 20 * leaseArea.value;
          let appliedLeaseAmount = (75 / 100) * feePaidAmount.value;
          let totalAmount = (areawiseValue > appliedLeaseAmount) ? appliedLeaseAmount : areawiseValue;
          let totalAreaPerEmp = parseFloat(leaseArea.value) / parseInt(intTotalEmpCount);
          if(totalAreaPerEmp <= 30){
            totalRentalAmount.value = totalAmount;
          }else{
            totalRentalAmount.value = '';
            Swal.fire({
              icon: 'error',
              text: 'Your per employee area is more than 30 sqft!'
            });
          }
        }
      }
  ...
  */
  customValidations(addMorectrlId){
   ...
   // console.log(this.intProductId,addMoreElement.classList);
    if((this.intProductId == 26 && addMoreElement.classList.contains('cls_other_assets')) || (this.intProductId == 110 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 102 && addMoreElement.classList.contains('cls_prev_claim_hd')) || (this.intProductId == 111 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 29 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 30 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 31 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 104 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 32 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 103 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 112 && addMoreElement.classList.contains('cls_parrent_previous_claim')) || (this.intProductId == 37 && addMoreElement.classList.contains('cls_parrent_previous_claim'))){
      let suiAddMoreId       = this.arrAddmoreDetails[addMorectrlId][0].ctrlId;
    ...
    } else if(this.intProductId == 111 && addMoreElement.classList.contains('cls_certifying_body')){
      // this.skillUpgradeAddmoreCount++;
      console.log(this.skillUpgradeAddmoreCount);
      if(this.skillUpgradeAddmoreCount >= 5){
        Swal.fire({
          icon: 'error',
          text: 'You can add maximun of 5 fields!'
        })
        return false;
      } else {
        this.skillUpgradeAddmoreCount++;
  ...
  finalApplyCustomValidations(){
    let uploadFile: any;
    let customValidatonStatus = true;
    let schemeWiseFormDetails = this.dynamicCtrlDetails[this.currSecTabKey]['formDetails'];
    for (let schemeWiseFormCtr of schemeWiseFormDetails) {
      let ctrlTypeId = schemeWiseFormCtr.ctrlTypeId;
      let elmId      = schemeWiseFormCtr.ctrlId;
      let elmClass   = schemeWiseFormCtr.ctrlClass;
      let lblName    = schemeWiseFormCtr.ctrlLabel;
      if(this.intProductId == 31 && elmClass == 'cls_intern_d_none' && ctrlTypeId == 7){
        uploadFile = this.arrUploadedFiles[elmId];
        if(uploadFile){
          console.log('if cse');
          if(uploadFile['fileName'] == '' || uploadFile['fileName'] == undefined) {
            ...
            customValidatonStatus = false;
            break;
          } else {
            customValidatonStatus = true;  // File uploaded successfully
        }
        }else{
          console.log('elsr cse');
          Swal.fire({
            icon: 'error',
            text: 'Please upload ' + lblName
          });
